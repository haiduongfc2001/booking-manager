...
const HotelAmenities = ({ hotelId, amenities = [] }) => {
  ...
  const handleDeleteAmenity = async (amenity_id) => {
    ...
    try {
      const response = await HotelService[API.HOTEL.AMENITY.DELETE_AMENITY]({
        hotel_id: hotelId,
        amenity_id,
      });
  ...
  const handleAddNewAmenity = async () => {
    dispatch(openLoadingApi());
    try {
      const response = await HotelService[API.HOTEL.AMENITY.CREATE_AMENITY](newAmenity);
      if (response?.status === STATUS_CODE.CREATED) {
        dispatch(showCommonAlert(TOAST_KIND.SUCCESS, response.message));
        setEditableAmenities([...editableAmenities, response.data]);
        setNewAmenity({ hotel_id: hotelId, amenity: "" });
      } else {
        const errorMessage =
          typeof response.data.error === "string"
            ? response.data.error
